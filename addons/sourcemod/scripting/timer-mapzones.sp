...
CheckVelocity(client, type, Float:maxspeed)
{
	if(maxspeed > 0.0)
	{
		new Float:fVelocity[3];
		...
		if(type == 0)
		{
			fVelocity[0] = (fVelocity[0]*(-1.0));
			fVelocity[1] = (fVelocity[1]*(-1.0));
			fVelocity[2] = (fVelocity[2]*(-1.0));
			
			new Float:diff = (SquareRoot(Pow(fVelocity[0],2.0)+Pow(fVelocity[1],2.0)))-500.0;
			...
			if(diff > 0.0)
			{
				while(diff > 0.0)
				{
					...
					{
						fVelocity[0] = fVelocity[0]/1.1;
						fVelocity[1] = fVelocity[1]/1.1;
						fVelocity[2] = fVelocity[2]/1.1;
					}
					else if(diff > 10.0)
					{
						fVelocity[0] = fVelocity[0]/1.01;
						fVelocity[1] = fVelocity[1]/1.01;
						fVelocity[2] = fVelocity[2]/1.01;
					}
					else
					{
						fVelocity[0] = fVelocity[0]/1.001;
						fVelocity[1] = fVelocity[1]/1.001;
						fVelocity[2] = fVelocity[2]/1.001;
					}
					
					new Float:newspeed = SquareRoot(Pow(fVelocity[0],2.0)+Pow(fVelocity[1],2.0));
					diff = newspeed-500.0;
				}
			}
			else
		...
		else if(type == 1)
		{
			...
			{
				new Float:diff = currentspeed - maxspeed;
				...
				if(GetGameMod() == MOD_CSGO)
				{
					PrintHintText(client, "Speed is limited at this area!");
				}
				else PrintCenterText(client, "Speed is limited at this area!");
			}
		}
		// 2: bounce back
		else if(type == 2)
		{
			...
			new Float:diff = (SquareRoot(Pow(fVelocity[0],2.0)+Pow(fVelocity[1],2.0)))-1.5*maxspeed;
